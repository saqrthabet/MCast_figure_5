function [PSNRgop,noise,uv,g]=mtime_latgopfml(filename,noise,H,SNR_table,fln,gopcnt,skip,tslots_num,blk_num_i,blk_num_j,gopbg,goptmt)
%% parameter setting
SNR_list=SNR_table;
...
outname=strcat('outsnr', int2str(SNR_table) ,'ts',int2str(tslots_num),'_softcast.yuv');
%% Digital Modulation 
        ...
        for gcac=1:tslots_num     % the iteration depends how many time slots we have 
              ...
              I_rec1 = zeros(n_width, n_height*gopcnt);    
                 for bb=1:block_totnum
                     ...
                     mmse_w=sigma_scur*(Hgi)'/w2;    %sigma_scur*(Hgi)'/w2;                                 % MCast equ(3)without Yi
