function [PSNRgop,noise,uv,g]=mtime_latgopfml_joint(filename,noise,H,SNR_table,fln,gopcnt,skip,tslots_num,blk_num_i,blk_num_j,gopbg,goptmt)
%% parameter setting
SNR_list=SNR_table;
...
outname=strcat('outsnr', int2str(SNR_table) ,'ts',int2str(tslots_num),'_softcast_.yuv');
%% Digital Modulation
...
for hhcac=1:tslots_num                % Prepare channel Gain based on time slots
    htx(:,hhcac)=H(:,hhcac);  %hh              % Power allocation by treating H as parameters
    %htx(:,hhcac)=sort(hh,'descend'); % SoftCast does not Order Channel Gain 
end 
...
for iSNR = 1:length(SNR_list)
    ...
    for gop=gop_nobg:16:goptmt 
        ...
        %noise=gnoise(sigma_noise,SNR_list,n_width,n_height,gopcnt,blk_num_i,blk_num_j);
       ...
       %% 1D-DCT Temporal Decorrelation
        ...
        show_frames_FD(I3D,gopcnt,2); %SAQR
     ...
     % so now each chunk is not gathering the neighboring coeff. but chunks size is 4x12 instead of 88x96. But eventually it is similar to mine
        ...
        for gcac=1:tslots_num     % the iteration depends how many time slots we have 
              ...
              htxtt=htx(:,gcac);  % we only have 1 48-channel gain stock already reorder
               % Scaling factor
              gfml=gpafml_soft(htxtt,lumbda,powertot,block_totnum,hbidx); %lumbda_sorted
              g(:,gcac)=gfml;

              I_rec1 = zeros(n_width, n_height*gopcnt);    
                 for bb=1:block_totnum
                     curblk_index=hbidx(bb); % sortblk_idx index of real locations of the reorderred chunks
                     ...
                     sigma_scur=lumbda(bb); % lumbda_sorted
                     ...
                     e1=noise(loc_blkb.*gcac,:);  % When tslots_num=2, AWGN become just the even lines which is not sufficient, w
                     y1(bb,:,gcac) = Hgi*m + e1;     % H*G*R*X+AWGN    the received signal at the first receive antenna
                                                  % Yi=(Hi*Gi)*Xi+Zi MCast equation(2)
                 end
        end
                     % SoftCast+Joint 
                     % the receiver jointly decodes the received signals of all time slots.
                     for gcac=1:tslots_num 
                       g11(1+((gcac-1)*block_totnum):((gcac-1)+1)*block_totnum,1)=g(:,gcac); %gtotxtmp
                       htxtt11(1+((gcac-1)*block_totnum):((gcac-1)+1)*block_totnum,1)=htx(:,gcac);
                       y11(1+((gcac-1)*block_totnum):((gcac-1)+1)*block_totnum,1:block_size)=y1(1:block_totnum,1:block_size,gcac);
                       hbidx11(1+((gcac-1)*block_totnum):((gcac-1)+1)*block_totnum,1)=hbidx(:,1);
                     end
                     
%                      y11=sum(y1,3)./tslots_num;
%                      htxtt11=sum(htx,2)./tslots_num;
%                      g11=sum(g,2)./tslots_num;
                 ...
                 for bb=1:block_totnum
                     ...
                     sigma_scur=lumbda(bb); % lumbda_sorted
                     loc_blkb=find(hbidx11==bb); % decides what G,H,AWGN I selected for each specific chunk
                     
                     x_bblk=x(loc_i:(loc_i+blk_size_i-1), loc_j:(loc_j+blk_size_j-1)); % Chunk content
                     xx_blk=x_bblk(:)';
                     
                     gcur=g11(loc_blkb);     % Gi
                     htxcur= 1;%htxtt11(loc_blkb); % Hi
                     
                     Hgi= htxcur.*gcur; %H*G*R  
                     w1=sum(Hgi.^2);   % gcur^2; %|H.G|^2 The Sum is for establishing bij=sum(Hgi(loc_blkb,:) w1=sum(Hgi(loc_blkb,:).^2);
                     w2=w1*sigma_scur+sigma_noise^2;  %lambda*|H.G|^2+n_std^2
                     mmse_w=sigma_scur*(Hgi)'/w2;    %sigma_scur*(gcur)'/w2;                                 % MCast equ(3)without Yi
                     mse_mlatheory(iSNR,bb,gop_no)=sigma_noise^2/(w2/sigma_scur); % MCast equ(4)
                     
                     xx_dec=mmse_w*y11(loc_blkb,:);    %bb           % MCast equ(3) complete 
                     xx_dec=xx_dec';                              
        ...
        %end    %for every additional time slot
         ...
         %% 1D-iDCT Temporal Decorrelation
                    for i=1:w
                        for j=1:h
                            I3Dfl(i,j,:)=idct(I3D(i,j,:)); % I3Dfl=idct(I3Dfl(i,j,:));
                        end
                    end
